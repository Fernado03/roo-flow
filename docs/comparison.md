...
A few projects in the agentic-coding space share vocabulary with Roo
...
- **Roo Flow** is a Roo / Zoo-native workflow control plane. The center
  of gravity is the *contract*: three modes, a routing matrix, a command
  protocol, and a path-safety guarantee, validated by a small smoke-test
  suite.

These projects are complementary more than competitive. You can run a
broad skills library inside Roo Flow's three-mode contract, and you can
plug Roo Flow's commands into a richer harness if you have one.

## What Roo Flow is not
...
- It is not a replacement for a methodology like Superpowers. If you
  ...
  coding, look there.
- It is not a runtime. Roo Flow runs inside Roo Code or Zoo Code; it
  does not ship its own agent loop, memory store, or tool registry.
...
If you only adopt one piece of Roo Flow, adopt the path-safety rule and
the command protocol. Those two files alone fix most of the "agent
hallucinated a path" problems.

## When to pick Roo Flow

- You are using Roo Code or Zoo Code as your primary host.
...
- You are not on Roo Code / Zoo Code and you do not plan to be. Roo
  Flow leans on `customModes`, `run_slash_command`, `new_task`, and
  `switch_mode` semantics from those hosts.
...
Roo Flow plays well with broader skills libraries. The skills under
...
problem space. Roo Flow exists because their work raised the bar.
Where Roo Flow disagrees with them, it disagrees on tradeoffs, not on
intent.
